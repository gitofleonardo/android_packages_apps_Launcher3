...
public class OverScroller {
    ...
    public void startSpringScroll(int startX, int startY, int dx, int dy, float velocityX, float velocityY) {
        mMode = SCROLL_MODE;
        mScrollerX.mState = SplineOverScroller.SPRING;
        mScrollerX.startSpringScroll(startX, dx, velocityX);
        mScrollerY.mState = SplineOverScroller.SPRING;
        mScrollerY.startSpringScroll(startY, dy, velocityY);
    }
    ...
    public boolean isSpringing() {
        return mScrollerX.mState == SplineOverScroller.SPRING ||
                mScrollerY.mState == SplineOverScroller.SPRING;
    }
    ...
    static class SplineOverScroller {
        ...
        private static final FloatPropertyCompat<SplineOverScroller> SPRING_PROPERTY =
                new FloatPropertyCompat<SplineOverScroller>("splineOverScrollerSpring") {
                    @Override
                    public float getValue(SplineOverScroller object) {
                        return (float) object.mCurrentPosition;
                    }
        ...
        void startSpringScroll(int start, int distance, float velocity) {
            mFinished = false;
            ...
            mSpringAnimation = new SpringAnimation(this, SPRING_PROPERTY);
            mSpringAnimation.setSpring(new SpringForce(mFinal)
                    .setStiffness(300)
                    .setDampingRatio(.8f));
            mSpringAnimation.setStartValue(mStart);
            mSpringAnimation.setStartVelocity(velocity);
            mSpringAnimation.addEndListener((animation, canceled, value, velocity1) -> {
                mSpringAnimation = null;
                finish();
                mState = SPLINE;
            });
        ...
        void setFinalPosition(int position) {
            mFinal = position;
            if (mState == SPRING && mSpringAnimation != null) {
                mSpringAnimation.animateToFinalPosition(mFinal);
            }
